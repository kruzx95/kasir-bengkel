...
export default function ImportSparepartModal({ open, onClose, branches }: ImportSparepartModalProps) {
  ...
  const isServiceFileName = file?.name.toLowerCase().includes('servis') || file?.name.toLowerCase().includes('service')
    ...
    <Modal open={open} onClose={handleClose} title="Import Sparepart dari Excel">
      <div className="p-6 space-y-5">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          ...
          >
            Unduh Template Excel (Sparepart)
          </Button>
        </div>
            ...
            <div className="space-y-2">
              ...
              {isServiceFileName && (
                <div className="flex items-center gap-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-xs">
                  ...
                  <span>Perhatian: Nama file mengandung &apos;servis&apos;. Pastikan file ini berisi data <strong>Sparepart</strong> (kolom nama & harga_jual).</span>
                </div>
              )}
            </div>
          )}
